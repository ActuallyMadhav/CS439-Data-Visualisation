...
# Default attribute combinations for the two panels (free choice, per
# the assignment - chosen to double as a demo of two different
# attribute groups, as in the assignment's example screenshots).
...
class BubblePanel(QtWidgets.QWidget):
    ...
    def __init__(self, df, attributes, defaults, on_brush, parent=None):
        ...
        self.last_sub = None  # last drawn (cleaned) subset, incl. 'name'
...
class BrushingWindow(QtWidgets.QMainWindow):
    def __init__(self, filename):
        super().__init__()
        self.setWindowTitle('HW2 - Task 3: Linked Brushing')

        self.df = load_data(filename)
        attributes = get_numeric_attributes(self.df)
        self.selected_names = None  # None/empty == no active selection

        central = QtWidgets.QWidget()
        self.setCentralWidget(central)
        layout = QtWidgets.QHBoxLayout(central)

        self.left_panel = BubblePanel(self.df, attributes, LEFT_DEFAULTS, self.handle_brush)
        self.right_panel = BubblePanel(self.df, attributes, RIGHT_DEFAULTS, self.handle_brush)
...
def task3(filename):
    ...
    window = BrushingWindow(filename)
    window.show()
    app.exec_()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Linked Brushing')
